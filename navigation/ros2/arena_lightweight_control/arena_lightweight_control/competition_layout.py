...
맵 좌표계: 벽이 -ARENA_HALF_M..+ARENA_HALF_M, 변환 = cm/100 - ARENA_HALF_M
(grid_match_strategy.md 단계 A1 — 2026-07-15 실기 검증 완료: 출발구역
포즈 시딩 후 실제 태극기 코너 = UI STORAGE 방향 일치 확인).
"""

ARENA_HALF_M = 1.0   # [demo/arena-2m] 경기 2.0

# [demo/arena-2m] 피치 50cm 유지, 점 개수만 축소 (7x6=42 -> 3x2=6).
GRID_XS_CM = tuple(range(50, 151, 50))   # 3개
GRID_YS_CM = tuple(range(100, 151, 50))  # 2개
...
STORAGE_RECT_MAP = (*official_cm_to_map(0, 0),
                    *official_cm_to_map(40, 40))    # 좌하단, 태극기
START_RECT_MAP = (*official_cm_to_map(ARENA_HALF_M * 200 - 40, 0),
                  *official_cm_to_map(ARENA_HALF_M * 200, 40))   # 우하단
...
# [demo/arena-2m] 스캔 지점 = 출발 포즈. 사유는 fieldlib.CENTER_SCAN_XY 주석.
CENTER_SCAN_MAP = official_cm_to_map(ARENA_HALF_M * 200 - 20.0, 20.0)
